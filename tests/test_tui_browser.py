...
@pytest.mark.asyncio
async def test_run_browser_successful_refresh_restores_focused_highlighted_run(
    tmp_path: Path,
) -> None:
    _persist_run(tmp_path, "older", updated_at="2026-01-01T00:00:00+00:00")
    _persist_run(tmp_path, "selected", updated_at="2026-01-02T00:00:00+00:00")
    ...
    async with app.run_test() as pilot:
        ...
        assert run_list.index == 1


@pytest.mark.asyncio
async def test_run_detail_successful_refresh_restores_focused_record_cursor(tmp_path: Path) -> None:
    run_id = "selected"
    _persist_run(tmp_path, run_id, updated_at="2026-01-02T00:00:00+00:00")
    _persist_history(tmp_path, run_id, "0001-planning.json", seq=1)
    snapshot = build_snapshot(ProjectResolution(integration_root=tmp_path, git_common_dir=tmp_path))
    app = RunBrowserApp(snapshot)

    async with app.run_test() as pilot:
        await pilot.press("enter", "down", "r")

        record_list = app.screen.query_one("#record-list", ListView)
        assert app.screen.focused is record_list
        assert record_list.index == 1
