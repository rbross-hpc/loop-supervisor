...
def test_verifying_runs_every_command_even_after_an_earlier_failure(tmp_path):
    # Unlike provisioning, verification must never stop early: a lint
    # failure should not hide a downstream test failure the auditor also
    # needs to see.
    runner = ScriptedRunner(
        {
            "loop-planner": [_planner_ready()],
    ...
    options = _make_options(verify_commands=("sh -c 'exit 1'", "echo second-ran"))
    supervisor, repo = _make_supervisor(tmp_path, runner, options=options)
    state = supervisor.start_new_run()
    supervisor.advance(state)
    supervisor.advance(state)
    ...
    assert commands[0]["ok"] is False
    assert commands[1]["ok"] is True
    assert "second-ran" in commands[1]["summary"]


def test_verification_logs_are_written_outside_the_task_worktree(tmp_path):
    runner = ScriptedRunner(
        {
            "loop-planner": [_planner_ready()],
            "loop-builder": [_builder(status="COMPLETE")],
    ...
    commands = state.verification_result["commands"]
    log_path = Path(commands[0]["output_path"])
    assert log_path.is_absolute()
    worktree_path = Path(state.task_worktree_path)
    assert worktree_path not in log_path.parents
    assert log_path.exists()
    assert "hello-verification" in log_path.read_text()
...
def test_verification_does_not_dirty_the_task_worktree_without_a_loop_supervisor_gitignore_entry(
    ...
    # Regression test: an earlier version wrote verification logs under
    # the task worktree, relying on `_skeleton/.gitignore` reserving
    # `.loop-supervisor/`. That guarantee only holds for projects
    # generated by `init`; an *adopted* project's `.gitignore` may not
    # have that entry. Verify the worktree stays clean even then.
    runner = ScriptedRunner(
        {
            "loop-planner": [_planner_ready()],
            "loop-builder": [_builder(status="COMPLETE")],
    ...
    worktree_path = Path(state.task_worktree_path)
    assert repo.is_clean(cwd=worktree_path)
